...
**用途定位（用户拍板）**：学习深度优先 + 演示分享兼顾——每个 Ontology 机制亲手实现一遍；数据小而真（默认取沪深300成分中总市值前 50 只 + 申万一级行业，经本机金融技能拉真实数据，后续可增删），不追求每日更新；界面要有观感、能承载概念叙事。
...
|---|---|---|---|
| Stock | 股票代码 | 名称、行业码、最新价、市值、PE、PB、自选标记 | stocks.csv（技能拉取） |
| Industry | 行业码 | 申万一级行业名 | industries.csv |
...
刷新行情数据集 → Funnel 增量物化 → Web 对象浏览器看 Stock，沿链接跳行业/持仓 → 跑 screenStocks 筛候选 → 对某股 writeResearchNote + tradeStock（校验通过，持仓即时更新，审计流 +2）→ Claude Code 对话"把 XX 加自选并设预警" → Claude 经 MCP 调 Action 完成 → UI 刷新可见。
...
- 单测（vitest）：schema 校验、物化幂等、合并策略、Action 拒绝路径、Function 求值。
- 端到端：种子数据 → 调仓 → 断言合并视图与审计。
- 错误处理：schema 加载 fail-fast 带定位；Action 失败返回结构化原因（哪条 criteria 未过）；Funnel 脏行跳过并出报告，不静默丢弃。
